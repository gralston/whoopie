...
function WhoopieTrickConstructor()  {
    ...
    this.trumpSuit = "";            // the current trump suit DEPRECATED just use stanza version
...
// var AllStanzas = []; // all the stanzas played NOT USED - see stanzas in whoopiestatus
...
function WhoopieStanzaConstructor()  {
    ...
    this.allHiddenCards     = [];       // all the cards hidden (tricks taken) in this stanza for easy access to show them again
...
var WhoopieStatus = {
    ...
    state: "",             // waitingForGame, choosingFirstDealer, gameOver, newdeal, bidding, tricktaken, playing
    waitingForGame : true,
    ...
    verifyDeck: function() {
        var cards = [];
        cards.length = 54; cards.fill(0);

        for (i=0; i<this.deckIndex.length;i++) {
            
            if (cards[this.deckIndex[i]] == 1)
                alert("Deck has a problem: duplicate card index: ", this.deckIndex[i]);
            else
                cards[this.deckIndex[i]] = 1;
        }
    } 
}
...
function joinGame(gameName, firstid) {
    ...
    function proceed(){
        ...
        initializeWhoopie();
        $("#whoopieMessageDiv2").css("display", "block");
        $("#whoopieScore").css("display", "block");
...
function chooseDealer() {
    ...
    setTimeout(finishChooseDealer, 5000);  
...
function yourDeal(handNumber) {
    ...
    function proceed(){
       ...
       // dealWhoopieHand(handNumber, false);      // don't deal here, deal when you get the newDeal request - but if the hand we 
                                                   // deal is wrong, ma
        
        WhoopieStatus.deckToIndex();
        WhoopieStatus.verifyDeck();
...
function dealWhoopieStanza(handNumber, faceUp) {
    ...
    WhoopieStatus.stanzas[handNumber].clear();
    ...
    for (var i = 0; i < WhoopieStatus.numPlayers; i++) {
        ...
        $("#bidID"+i).html("");         // clear bids from previous stanza
        $("#trickID"+i).html("");       // clear tricks taken from previous stanza
    }

    var id = WhoopieStatus.dealer;
    var next = Players[nextPlayer(WhoopieStatus.dealer, WhoopieStatus.numPlayers)];
    for (i = 0; i < WhoopieStatus.numPlayers; i++) {
        hands[i] = next.hands[handNumber].hand;
        next = Players[nextPlayer(next.playerID, WhoopieStatus.numPlayers)];
    }
    ...
    WhoopieStatus.stanzas[handNumber].cardsDealt = numCards;
     ...
     // deal hands and turn over whoopie card (unless this is the first deal chooser hand)
     if (handNumber != 0)
        WhoopieStatus.deck.deal(numCards, hands, 100, turnOverWhoopieCard);
     else {
        WhoopieStatus.deck.deal(numCards, hands, 100);
     }
...
function organizeHand(hand) {
    ...
    if (hand.length > 3)
        console.debug("organizehand: ", hand.length, hand[0].shortName, hand[1].shortName, hand[2].shortName, hand[3].shortName);
    ...
    diamonds.sort(function(a,b) { return(a.rank - b.rank)});
    ...
    var zIndexCounter = 1;
    for (var i = 0; i < hand.length; i++) {
        var newCard = newHand[i];
        var card = hand[i];
        var pos = $( newCard.el ).position();
        console.debug("organize hand - details - first card replaced by second card", card.shortName, newCard.shortName );
        console.debug("organize hand - details - moving card from ", pos.top, pos.left );
        console.debug("organize hand - details - moving card to ", savedProps[i].top, savedProps[i].left );
        $(newCard.el).css(savedProps[i]);
        pos = $( newCard.el ).position();
        console.debug("organize hand - details - after move card is at:  ", pos.top, pos.left );
        $(newCard.el).css('z-index', zIndexCounter++);
    }

}

function turnOverWhoopieCard() {
    
    var whoopieDeck;
    whoopieDeck = new WhoopieStatus.cards.Deck({faceUp:true});
...
function organizeDealerHand() {
    organizeHand(ThisPlayer.hands[WhoopieStatus.handNumber].hand); 
}
    ...
    $.get($getURL, function(data,status) {
        ...
        for (i = 0; i < whoopieEvents.length; i++) {
            ...
            if (ev.type == "playerJoined") {
                ...
                // set variables which let you figure out when it's your turn
                if (WhoopieStatus.playerID == 0)
                    WhoopieStatus.playerBeforeMe = WhoopieStatus.numPlayers - 1;
                else
                    WhoopieStatus.playerBeforeMe = WhoopieStatus.playerID - 1;
                ...
                WhoopieStatus.state = "bidding";    // from now on we are bidding

            } else if (ev.type == "newBid") {
                whoopieMessageOff();    // clear dealer message (if there)
                
                $("#bidID"+ev.playerID).html(ev.bidMade);
                $("#trickID"+ev.playerID).html(0);
...
function updateTrickStatus(playerID, card) {
    ...
    console.debug("updateTrickStatus", CurrentStanza.trumpSuit, CurrentTrick.playerID, CurrentTrick.cardsPlayed, CurrentTrick.suitLed);
    ...
    if (CurrentTrick.cardsPlayed == WhoopieStatus.numPlayers) {
        ...
        WhoopieStatus.state = "tricktaken";
...
function undoCardIsTrump(card) {
    $(card.el).css({"border-width":"0px"});
}

function showCardIsTrump(card) {

    $(card.el).css({"border-color": "gold", 
    "border-width":"2px", 
    "border-style":"solid"});
...
function handleJoker(playerID, card) {
    ...
    if (CurrentTrick.cardsPlayed == 1) {
        ...
        CurrentStanza.trumpSuit = CurrentTrick.suitLed;
    }      
    CurrentTrick.winningCardisTrump = true;
}
...
function updateScore(stanza) {
    ...
    for (var i=0; i < WhoopieStatus.numPlayers; i++) {
        var bid = stanza.bids[i];
        var tricks = stanza.tricksTaken[i];
        if (bid == tricks) {
            stanza.scores[i] = tricks+2;
        } else
            stanza.scores[i] = -1;

        WhoopieStatus.scores[i] += stanza.scores[i];
    }
   
    updateScoreTable(stanza);
}

function updateScoreTable(stanza) {
    console.debug("updatescoretable - ", stanza);

    // var bidstable = document.getElementById("whoopieBidsTable");
    // var bidsbody = bidstable.tBodies[0];
    // var bidsrow = bidsbody.insertRow(-1);

    var scoretable = document.getElementById("scoreTable");
    var scorebody = scoretable.tBodies[0];
    var scorerow = scorebody.insertRow(-1);

    scorerow.insertCell(0).innerHTML = WhoopieStatus.handNumber;

    for (var i=0; i < WhoopieStatus.numPlayers; i++) {
        scorerow.insertCell(i+1).innerHTML = stanza.bids[i];
        $("#totalsID"+i).html(WhoopieStatus.scores[i]);
    } 
    scorerow.insertCell(WhoopieStatus.numPlayers+1).innerHTML = "";

    for (var i=0; i < WhoopieStatus.numPlayers; i++) {
        scorerow.insertCell(i+2+WhoopieStatus.numPlayers).innerHTML = stanza.scores[i];
    }     
...
function cleanUpHand(hand) {
    var len = hand.length;
    console.debug("cleanUpHand", len);
    for (var i = 0; i < len; i++) {
            var card = hand[i];
            undoCardIsTrump(card);
            pos = $(WhoopieStatus.deck[WhoopieStatus.deck.length-1].el).position();
            console.debug("cleanUpHand", card, i, pos.top, pos.left);
            var props = {
                //top: WhoopieStatus.deck.targetTop,
                //left: WhoopieStatus.deck.targetLeft,
                top: pos.top,
                left: pos.left,
                queue: false
            };
    ...
    //WhoopieStatus.deck.addCards(hand);
    //WhoopieStatus.deck.render({immediate:true});
}
function cleanUpLastStanza() {
    if (WhoopieStatus.handNumber == 1) {
        // clean up first deal (no tricks, no Stanza
        for (var i=0; i < WhoopieStatus.numPlayers; i++) {
            cleanUpHand(Players[i].hands[0].hand);
        }
    } else {
        cleanUpHand(CurrentStanza.allHiddenCards);
    }
    
}


function cleanUpLastStanzaOLD() {
    console.debug("cleanUpLastStanza: handNumber, tricksPlayed", WhoopieStatus.handNumber, CurrentStanza.tricksPlayed);
    if (WhoopieStatus.handNumber == 1) {
        // clean up first deal (no tricks, no Stanza
        for (var i=0; i < WhoopieStatus.numPlayers; i++) {
            cleanUpHand(Players[i].hands[0].hand);
        ...
        // 
        for (var i = 0; i < WhoopieStatus.handNumber - 1; i++) {
            console.debug("CleanUpLastStanza: CurrentStanza ", CurrentStanza);
            cleanUpHand(CurrentStanza.allTricks[i].hand);
        }
        CurrentStanza.whoopieCard.hideCard();
...
function takeTrick(playerID) {
     ...
     for (i = 1; i < trick.length; i++) {
         ...
         if (i == trick.length-1) { // last card, call back function to remove trick
             $(trick[i].el).animate(props,1000, function() {  // move all the cards together
                 
                 var playerElement = "#player" + playerID;

                 pos = $(playerElement).position();
                 var props = {
                     top: pos.top,
                     left: pos.left,
                     queue: false
                 ...
                 for (i = 0; i < trick.length; i++) {
                    ...
                    if (i == trick.length-1) {
                        $(trick[i].el).animate(props, 1000, function() {
                            ...
                            for (i = 0; i < trick.length; i++) {    
                                var card = trick[i];
                                card.hide();      // make it disappear
                                card.hideCard();  // also turn it over
                                CurrentStanza.allHiddenCards[CurrentStanza.allHiddenCards.length] = card;
                            }
                        });
                    } else 
                        $(trick[i].el).animate(props, 1000);
 ...
 */
SuitLongnames = {"d" : "diamonds", "s" : "spades", "c" : "clubs", "h" : "hearts"};
...
function playCard(playerID, card) {
    ...
    if (CurrentTrick != null && (card.suit != CurrentTrick.suitLed && playerHasSuit(playerID, CurrentTrick.suitLed))) {
        alert("You must follow suit (" + SuitLongnames[CurrentTrick.suitLed] + ") - if you can!");
        return(false);
    }
    WhoopieStatus.lastPlayerID = playerID;
    WhoopieStatus.state = "playing";
    ...
    // create a new trick
        ...
        CurrentTrick.hand = new WhoopieStatus.cards.Hand({faceUp:true, x:300, y:200});
        // CurrentTrick.trumpSuit = CurrentStanza.trumpSuit;
        CurrentTrick.suitLed = card.suit;
        CurrentStanza.allTricks[CurrentStanza.tricksPlayed] = CurrentTrick;   // we'll use this later to move the cards back to the deck
    }
    
    CurrentTrick.hand.addCard(card);
    CurrentTrick.hand.render();
    console.debug("playCard - added card to trick: length", CurrentTrick.hand.length);
 ...
 */
function myTurn() {
    console.debug("myTurn: (state myID, playerBeforeMe, lastPlayerID, tooktheirturn, alreadybid)", WhoopieStatus.state,
            WhoopieStatus.playerID, WhoopieStatus.playerBeforeMe,WhoopieStatus.lastPlayerID, 
            WhoopieStatus.thisPlayerTookTheirTurn, WhoopieStatus.thisPlayerAlreadyBid);

    var retVal = false;

    if (WhoopieStatus.state == "newdeal") {
        if (WhoopieStatus.dealer == WhoopieStatus.playerBeforeMe)
            retVal = true;
    } else if (WhoopieStatus.state == "bidding") {
        if (WhoopieStatus.lastPlayerID == WhoopieStatus.playerBeforeMe && !WhoopieStatus.thisPlayerAlreadyBid)
            retVal = true;
    } else if (WhoopieStatus.state == "tricktaken") {
        if (WhoopieStatus.hasLead == ThisPlayer.playerID)
            retVal = true;
    } else if (WhoopieStatus.state == "playing") {
        if (WhoopieStatus.lastPlayerID == WhoopieStatus.playerBeforeMe && !WhoopieStatus.thisPlayerTookTheirTurn)
            retVal = true;
    } else  
        retVal = false;

    console.debug("myturn returning: ", retVal);

    return(retVal);

    /*if (WhoopieStatus.hasLead == WhoopieStatus.playerID)
        return(true);     
    else if (WhoopieStatus.hasLead == -1 && WhoopieStatus.playerBeforeMe == WhoopieStatus.lastPlayerID) {
...
function getYourBid() {
    ...
    var numCards = handNumberToCards(WhoopieStatus.handNumber);

    console.debug("getYourBid (dealer, this, bidTotal, numCards)", WhoopieStatus.dealer, ThisPlayer.playerID, CurrentStanza.bidTotal, numCards);

    
    $('#whoopieBid').empty();   // remove previous possible bids.
    /*
     * Allow add legal bids (dealer can't make bids come out to number of cards dealt)
     */
    for (var i = 0; i <= numCards; i++) {
        if (WhoopieStatus.dealer != ThisPlayer.playerID || (CurrentStanza.bidTotal + i != numCards))
            $('#whoopieBid').append(`<option value="${i}"> ${i}</option>`);
    }  
    askForBid();
    /*
    var bid = Number(askForBid());
    while (bid > 0) { 
        alert("That's not a legal bid! Please try again.");
        bid = Number(askForBid());
    }
    */
}
/*
 * valid bid is anything less than the number of cards (that's enforced by the select, of course)
 * and it can't make the bids come out to the number of cards (I'm going to enforce that to anyway, I think)
 * 
 * DEPRECATED - pretty sure I don't need this because I only present valid bids (see getYourBid())
 */
function validBid(bid, numCards) {
    $(".ui-dialog-buttonpane button:contains('Proceed')").button("disable");

    if (!Number.isInteger(bid))
        return(false);
    else if (bid > numCards)
        return(false);
    else if (WhoopieStatus.dealer == ThisPlayer.playerID && CurrentStanza.bidTotal + bid == numCards)
        return(false);
    else
        return(true);
}
...
function askForBid() {
    ...
    function proceed(){
        ...
        var position = $( "#dialogMakeBid" ).dialog( "option", "position" );
        console.debug("dialogMakeBid position", position.top, position.left);
 

        // var numCards = handNumberToCards(WhoopieStatus.handNumber);

        whoopieSendRequest("newBid", bid, null);
        CurrentStanza.bidTotal += Number(bid);
        WhoopieStatus.thisPlayerAlreadyBid = true;
        $("#bidID"+WhoopieStatus.playerID).html(bid);
        $("#trickID"+WhoopieStatus.playerID).html(0);
        return(bid);          
...
function handleWhoopieResponse(resp) {
    ...
    } if (resp.name == "gameReset") {
        // do nothing, I think!
    } else if (resp.name.includes("Error")) {
        // assume an error
        alert(resp.name);
        window.location.href = "/whoopie.php";
    }
}
function whoopieTestStart() {
    var playerID;
    var playerName;
    ...
    WhoopieStatus.playerID = 1;
    WhoopieStatus.playerName = "Jon";
    whoopieSendRequest("joinGame", null, null);

    WhoopieStatus.playerID = 2;
    WhoopieStatus.playerName = "Steven";
    whoopieSendRequest("joinGame", null, null);

    WhoopieStatus.playerID = 3;
