...
var Convert = {
    type:   "SAFE",             // default, could also be "Note"
    totalInvested: 0,           // total invested using this security
    yourInvestment: 0,          // how much "you" invested
    cap: 0,                     // valuation cap
    discount: 0,                // discount, if any (10% will be 10)
    interestRate: 0,            // interest rate if any (I doubt I'll use this)
    price: 0,                   // calculated price / share when converted
    totalShares: 0,             // total number of shares when converted
    yourShares: 0,              // your number of shares when converted
    custom: "",                 // value pairs defining custom convertible attributes
    preexist: "",               // comma separated indices of preexisting converts
    equationNumerator: "",      // (documentation) numerator of equation used to calc. price
    equationDenominator: ""     // (documentation) numerator of equation used to calc. price
};

var Equity = {
    totalInvested: 0,           // total invested in this equity round
    yourInvestment: 0,          // how much, if any, you (the angel) invested in the round
    preMoneyVal: 0,             // pre money valuation
    //preOptionsPercent: 0,     // percent of pre-deal FD shares outstanding which are unissued options
    postOptionsPercent: 0,      // percent of post-deal FD shares outstanding which will be unissued options
    preOptionsPoolSize: 0,      // size of pre-money option pool
    preGrantedOptions: 0,       // total shares issued prior to round, but not included unallocated options
    prePromisedOptions: 0,      // weird stuff: shares promised to founders or others. may be larger than pool, still in prefd
    preUnallocOptions: 0,       // unallocated (unissued) pre-money options
    preFDShares: 0,             // fully diluted shares outstanding before the deal
    postFDShares: 0,            // fully diluted shares outstanding after the deal
    price: 0,                   // calculated price / share
    postOptions: 0,             // number of post money options
    totalShares: 0,             // total number of shares owned by equity
    yourShares: 0,              // number of angel shares if invested directly in equity
    totalYourShares: 0,         // total number of angel shares including investments in converts
    convertShares: 0,           // total number of convertible shares, when converted
    equationNumerator: "",      // (documentation) numerator of equation used to calc. price
    equationDenominator: "",     // (documentation) numerator of equation used to calc. price
    founderEquity: [],          // founder number: shares (for storage during delete)
    founderNames: [],           // founder number: name
    commonEquity: [],          // common number: shares  (for storage during delete)
    commonNames: [],           // common number: name
    numFounders: 0,
    numOtherCommon: 0,
    founderTotalEquity: 0,     // sum of founder equity
    commonTotalEquity: 0,     // sum of common equity not incl founders
    reset: function () {
            this.totalInvested      = 0;
            this.yourInvestment     = 0;
            this.preMoneyVal        = 0;
            this.postOptionsPercent =  0;
            this.preUnallocOptions =  0;
            this.preFDShares =  0;
            this.postFDShares =  0;
            this.price =  0;
            this.postOptions =  0;
            this.totalShares =  0;
            this.yourShares =  0;
            this.totalYourShares =  0;
            this.convertShares =  0;
            this.founderTotalEquity =  0;
            this.commonTotalEquity =  0;
            this.numFounders = 0;
            this.numOtherCommon = 0;
            this.convertShares =  0;
            this.equationNumerator =  "";
            this.equationDenominator =  "";
...
var FundPerformanceTotals = {
    ...
    clear: function() {
        ...
        this.carryOnlyCap        = 0;
        this.carryOnlyPredRet    = 0;
    }
}

function AngelcalcFundModel() {
    this.name           = "",           // name of the model - assuming models have names!
    this.amount         = 0,            // amount to be split over the years
    this.yearCount      = 10,           // years in model
    this.years          = {},           // for each year, fraction of total returned that year
//    this.left           = 0,            // fraction left for post yearCount years  DEPRECATED. 10 years is all of it

    this.amountInYear =  function(year) {
        // console.debug("amountinyear: (year, this.years[year]:" +  angelParens(year, this.years[year]));
        return(this.amount * this.years[year]);
    },
        /* DEPRECATED: decided all of the funds would have to be used in 10 years.
    this.amountLeft =  function() {
        return(this.amount * this.left);
    }*/
    this.defaultModel = function(inceptionYear) {
        // console.debug("angelcalcfundmodel.defaultmodel inceptionYear: " + inceptionYear);
        /*
         * the default model is to split the returns as follows (CHANGED THIS - NEED TO UPDATE)
         *      inceptionYear + 7: 5%
         *      inceptionYear + 8: 10%
         *      inceptionYear + 9: 70%
         *      inceptionYear + 10: 15%
         *
         * if no inception year is given, it is assumed to be the current year.
         * if inception year > 7 years ago, inceptionYear is incremented by 3 to make sure we get all
         * of the remaining distributions (in years 7-10)
         *
         */
        if (inceptionYear == null || inceptionYear == 0)
            inceptionYear = CurrentYear;
        if (CurrentYear - inceptionYear > 7)
            inceptionYear +=  3;

        var percentUsed = 0;
        for (var i = CurrentYear; i < (CurrentYear + this.yearCount); i++) {
            if (i ==  inceptionYear + 7)
                this.years[i] = .05;
            else if (i == inceptionYear + 8)
                this.years[i] = .1;
            else if (i == inceptionYear + 9)
                this.years[i] = .7;
            else if (i == inceptionYear + 10)
                this.years[i] = .15;
            else
                this.years[i] = 0;
            percentUsed += this.years[i];

        }
 /*       this.left = 1 - percentUsed;    DEPRECATED */
    },
    this.setModel = function(model) {
        var dict = parseQueryStringToDictionary(model);
        for (var i = 1; i <= this.yearCount; i++) {
            var year = CurrentYear + i -1;
            this.years[year] = Math.round(Number(dict[i]))/100;
        }
        // console.debug("angelcalcfundmodel.setmodel: years: " + JSON.stringify(this.years));
    }
}

/*
 * List of all companies and a list of all Investments for an Angel
 */
function InvestmentsConstructor()  {
    this.list           = [],
    this.count          = 0,
    this.totalInvested  = 0,
...
/*
 * companies and funds both use this object to store information about all fund and companies. notably, how much was
 * invested in it and how much it has returned and is predicted to return, and all the notes for the company / fund.
 */
function CompaniesConstructor() {
    this.map = {};                   // maps company id to the company object
    this.founders= {};              // maps company id to an array of founders
    this.totalInvested= {};         // maps company id to the amount invested in this company
    this.totalReturned= {};         // maps company id to the amount returned from investing in this company
    this.totalShares= {};           // maps company id to the total number of shares held in the company
    this.list= [];
    this.ycList = [];               // list of yc funds in YC order
    this.count = 0;
    this.jsonToList = function (data) {

        this.count = 0;
        var cos = JSON.parse(data);
        this.parseCompanies(cos);

    };
    this.parseCompanies = function (cos) {
        if (cos == null) {
            return;
        }
        this.list = [];     // reset to empty
        for (i = 0; i < cos.length; i++) {
            co = cos[i];
            co.exitSharePrice = -1,        // default indicates no exit prediction made for this co.
                co.predictedReturn = -1,       // default indicates no exit prediction made if a fund (ugh).
                co.capitalAccount = -1;        // ugh again (a fund)
            co.model = new AngelcalcFundModel();
            co.model.defaultModel(Number(co.inceptionYear));   // set up the default model
            // console.debug("jsontolist: JSON Co.model: " + JSON.stringify(co.model));
            this.map[(co.id).toString()] = co;
            this.list[i] = co;
            if ("nonAlphaKey" in co && co.nonAlphaKey == "yc")        // if this is a yc fund, put it in ycList in order
                this.ycList[Number(co.nonAlphaOrder)] = co;
            this.count++;
        }
        // console.debug("JSON Cos: " + JSON.stringify(this.list));
    };
    this.parseNotes = function(notes) {
        /*
         * go through the notes and add fd shares, valuations, and predicted exits to the appropriate companies.
         * A predicted exit price of -1 indicates no prediction given.
         */
        var d, year;
        var shares, companyID, note, inv;
        ...
        for (i = 0; i < notes.length; i++) {
            ...
            /*
             * skip notes for companies for which there are no longer investments
             */
            co = this.company(companyID);
            if (co != null) {
                // console.debug("*********parse company notes: length, co: " + angelParens(notes.length, co.name));
                if (note.type == "valuation update") {
                    /*
                     * store each company's val updates with the company for use later in graphing etc.
                     */
                    if (! ("valUpdates" in co) ) {
                        co.valUpdates = {};
                        // console.debug("here");
                    }
                    // console.debug("ok!: " + JSON.stringify(co));
                    d = new Date(note.noteDate);
                    year = d.getUTCFullYear();
                    co.valUpdates[year.toString()] = note.valuation;

                    //if (co.name == "Stripe") {
                        // console.debug("adding valupdate to company: (year, valupd.len) " + co.name + " " + angelParens(year, JSON.stringify(co.valUpdates)));
                   // }

                    co.FDshares = note.FDshares;
                    co.valuation = note.valuation;
                    if (note.sharePrice != 0 && note.sharePrice != null)
                        co.sharePrice = note.sharePrice;
                    else if (co.FDshares != 0 && co.FDshares != null)
                        co.sharePrice = co.valuation / co.FDshares;
                } else if (note.type == "exit prediction")
                    co.exitSharePrice = note.sharePrice;
                  else if (note.type == "stock split" && note.splitRatio > 0) {
                    console.debug("*********parse company notes: split: (co,ratio) " + angelParens(co.name, note.splitRatio));
                    co.sharePrice = co.sharePrice / note.splitRatio;
                    co.FDshares = co.FDshares * note.splitRatio;
                    // update every investment in this company to splitRatio x the number of shares.
                    for (var j = 0; j < Investments.list.length; j++) {
                        inv = Investments.list[j];
                        if (inv.company_id == companyID) {
                            console.debug("here: companyID = " + companyID);
                            shares = Number(inv.shares) * note.splitRatio;
                            inv.shares = shares;
                            shares = Number(inv.commonShares) * note.splitRatio;
                            inv.commonShares = shares;
                            shares = Number(inv.prefShares) * note.splitRatio;
                            inv.prefShares = shares;
                            inv.price_paid = Number(inv.price_paid) / note.splitRatio;
                        }
                    }
                }
                /*console.debug(".parseNotes: (Company, type) - (shares, val) - (sharePrice,exitSharePrice): " + angelParens(co.name, note.type) + " - " + angelParens(co.FDshares, co.valuation) + " - "
                    + angelParens(co.sharePrice,co.exitSharePrice));*/
            }

        }

    };
    this.parseFundsNotes = function(notes) {
        /*
         * go through the notes and add cap account / predictions to appropriate funds.
         * A predicted distribution of -1 indicates no prediction given.
         */
        // console.debug("this.parseFundsNotes: " + JSON.stringify(notes));
        for (i = 0; i < notes.length; i++) {
            note = notes[i];
            fundID = note.fundID;
            /*
            ...
            if (fu != null) {
                if (note.type == "valuation update") {
                    fu.capitalAccount = note.capitalAccount;
                    if  (fu.model.amount == 0)                  // if already set by prediction leave as is
                        fu.model.amount = fu.capitalAccount;
                } else if (note.type == "exit prediction") {
                    fu.predictedReturn = note.predictedReturn;
                    fu.model.amount = fu.predictedReturn;
                }

                // console.debug("this.parseFundsNotes: (Fund, type) - (capAcct, PredRet): " + angelParens(fu.name, note.type) + " - " + angelParens(fu.capitalAccount, fu.predictedReturn));
            }

        }

    };
    this.company = function (id) {
        if (!(id in this.map)) {
            console.log("company requested which does not exist. Id=" + id);
            return(null);
        } else {
    ...
    this.fund = function (id) {     // we are used for funds too, and this seems a bit nicer :)
        return this.company(id);
    }
    this.invested = function (id) {        // return total amount invested in input company id

        if (id in this.totalInvested)
            return(this.totalInvested[id]);
        else {
            var invested = 0;
            var shares = 0;
            var i;
            for (i = 0; i < Investments.list.length; i++) {
                if (Investments.list[i].company_id == id) {
                    invested += Number(Investments.list[i].invest_amount);
                    shares += Number(Investments.list[i].shares);
                }
            }
            this.totalInvested[id] = invested;
            this.totalShares[id] = shares;
            // console.debug("invested: " + invested);
            return(invested);
        }

    };
    this.shares = function (id) {        // return total of shares held in input company id

        if (id in this.totalShares)
            return(this.totalShares[id]);
        else {
            var invested = 0;
            var shares = 0;
            var i;
    ...
    this.firstInvestment = function (id) {        // return first investment in input company id
        var foundInv = null;
        var foundDate = "";
        var dNew, dOld;
        var invested = 0;
        ...
        for (i = 0; i < Investments.list.length; i++) {
            if (Investments.list[i].company_id == id) {
                if (foundInv == null) {
                    foundDate = Investments.list[i].inv_date;
                    foundInv = Investments.list[i];
                } else {
                    var newDate = Investments.list[i].inv_date;
                    dNew = new Date(newDate);
                    dOld = new Date(foundDate);

                    if (dNew < dOld) {      // the newest inv is older
                        foundDate = newDate;
                        foundInv = Investments.list[i];
                    }
                }
    ...
    this.isProspect = function (id) {        // return true if the input company is a prospect and not a current inv.
    ...
    this.returned = function (id, payouts) {        // return total amount returned from input company id

        if (id in this.totalReturned)
            return(this.totalReturned[id]);
        else {
            var returned = 0;
            var i;
            for (i = 0; i < payouts.list.length; i++) {
                ...
                if (po.companyID == id) {
                    if (po.type == "cash")
                        returned += Number(po.cash);
                    else
                        returned += Number(po.shares * po.sharePrice);
                }
            }
            this.totalReturned[id] = returned;
    ...
    this.investments = function (id) {        // return an array of investments in input company id
        ...
        for (i = 0; i < Investments.list.length; i++) {
            if (Investments.list[i].company_id == id) {
                invests.push(Investments.list[i]);
            }
        }
...
/*
 * List of all payouts (=distributions) for an Angel
 */
function PayoutsConstructor() {
    this.list                   = [];
    this.count                  = 0;
    ...
    this.findByCompany = function(id) {
        var clist = [];
        var cnt = 0;
        for (i=0; i<this.list.length; i++) {
            if (this.list[i].companyID == id) {
                cnt++;
                clist[i] = this.list[i];
            }
        }
    ...
    this.basisToMap = function(id) {
        po = this.findByID(id);
        if (po.basis == 0 || po.basis == null || po.basis == "" )
            return(null);
        else
            return(parseQueryStringToDictionary(po.basis));
    }
}
...
var AngelTotals, AngelTotalsFiltered, FundTotalsInv, FundTotalsCarry;
...
function FundTotalsConstructor()  {

    TotalsConstructor.call(this);

    this.funds          = {};      // fund names mapped to returns by year (for the past pastYears and future futureYears)
    this.pastYears      = 10;       // we only go this many years in the past - must match table definitions in angelinvests
    this.futureYears     = 10;
    this.carryOnlyVal =  0;        // for funds only, the current capital account of carry only funds
    this.carryOnlyPredVal = 0;     // for funds only, the predicted return of carry only funds

    this.clear = function() {
        // TotalsConstructor.clear.call(this); // I'm not sure this is correct...
    ...
    this.initialize = function() {
        for (var y=CurrentYear - 25; y < CurrentYear + 25; y++) {
            this.years[y] = Object.create(oneYearTotals);
            this.invsPerYear[y] = 0;
        }

        /*
         * create an object for to store years data for each fund.
         */
        for (f in Funds.map) {
            fund = Funds.map[f];
            if (!this.funds.hasOwnProperty(fund.id)) {
                this.funds[fund.id] = {};
            }
        }


        this.countFundPayouts();
        this.countFundPayoutsFuture();
        this.countFundInvestments();

        this.calcIandR();
        this.calcValue();
    }
    /*
     * count distributions per year for all matching funds. also add to each fund's per year history
     */
    this.countFundPayouts = function() {

        var d, year, i, fundyears, amount;

        for (var i=0; i<FundPayouts.list.length; i++) {
            var p = FundPayouts.list[i];
            if (!this.checkFundsFilter(Funds.map[p.fundID]))        // only add the fund if it is not filtered
                continue;

            d = new Date(p.payDate);
            year = d.getUTCFullYear();
            year = year.toString();

            if (!this.years.hasOwnProperty(year))               // this would be a bug, should not happen
                continue;

            if (!this.funds.hasOwnProperty(p.fundID)) {
                fundyears = this.funds[p.fundID] = {};
            } else {
                fundyears = this.funds[p.fundID];
            }
            if (!fundyears.hasOwnProperty(year)) {
                fundyears[year] = Object.create(oneYearTotals);
            }

            if (p.type == "cash") {
                amount = Number(p.cash);
                this.years[year].return_amount += amount;
                fundyears[year].return_amount += amount;
            } else {
                amount =  Math.round(Number(p.shares)* Number(p.sharePrice));
                this.years[year].return_amount += amount;
                fundyears[year].return_amount += amount;
            }
    ...
    this.countFundPayoutsFuture = function() {

        var d, year, i, fundyears, fund;
        ...
        for (var i=0; i<Funds.list.length; i++) {
            ...
            if (!this.checkFundsFilter(fund))        // only add the fund if it is not filtered
                continue;

            //console.debug("coundFundPayoutsFuture: (fund.name, this.filter): " + angelParens(fund.name, this.filter));
            if (!this.funds.hasOwnProperty(fund.id)) {
                this.funds[fund.id] = {};
            }
            fundyears = this.funds[fund.id];

            /*
             * Need to do investments / capital payments (will get to this later)
             */

            /*
             * predicted returns
             */
            for (var y = CurrentYear; y < CurrentYear + this.futureYears; y++) {

                if (y == CurrentYear)
                    y = CurrentYear + 100;     // this is a hack because we'll have actuals and futures in this year

                if (!fundyears.hasOwnProperty(y)) {
                    fundyears[y] = Object.create(oneYearTotals);
                    // console.debug("coundFundPayoutsFuture: adding year (fund.name, y): " + angelParens(fund.name, y));
                }


                if (!this.years.hasOwnProperty(y)) {                    // should never happen thanks to inintialize
                    this.years[y] = Object.create(oneYearTotals);
                }
                if (y == CurrentYear + 100) {       // hack for current year
                    fundyears[y].return_amount = fund.model.amountInYear(y - 100);            // per fund total
                    this.years[y].return_amount += fund.model.amountInYear(y - 100);
                    y -= 100;
                } else {
                    fundyears[y].return_amount = fund.model.amountInYear(y);            // per fund total
                    this.years[y].return_amount += fund.model.amountInYear(y);
                }

            }
        }
    };       // countFundPayoutsFuture

    this.countFundInvestments = function() {
        var d, year, i, fundyears, f, fund;

        // console.debug("*****here: list length: " + Investments.list.length);
        for (i=0; i<FundInvestments.list.length; i++) {
            f = FundInvestments.list[i];

            fund = Funds.map[f.fundID];
            if (fund == null)        // only happens if we have a bug!
                continue;

            if (!this.checkFundsFilter(fund)) {       // only add the fund if it is not filtered
                // console.debug("countFundInvestments: filtering: (filter, fund.id): " + angelParens(this.filter, fund.name));
                continue;
            }
            ...
            if (f.type == "invest") {
                ...
                if (!this.years.hasOwnProperty(year)) {
                    this.years[year] = Object.create(oneYearTotals);
                }
                this.years[year].invest_amount += Number(f.investAmount);
                fundyears[year].invest_amount += Number(f.investAmount);
            }

        };

    };      // countFundInvestments
    ...
    this.calcValue = function(type) {
        ...
        for (var f in Funds.map) {
            ...
            if (fund.status == "active") {
                if ("capitalAccount" in fund && fund.capitalAccount > 0)
                    val += Math.round(Number(fund.capitalAccount));
                if ("predictedReturn" in fund && fund.predictedReturn > 0)
                    predval += Math.round(Number(fund.predictedReturn));
            }
        }

        // console.debug("this.calcCurrentValue (funds) (val, predval): " + angelParens(Math.round(val), Math.round(predval)));
        this.currVal = Math.round(val);
        this.predVal = Math.round(predval);

    };

    this.checkFundsFilter = function(fund) {

        // console.debug("checkFundsFilter: " + JSON.stringify(fund));
        if (this.filter == "all" ||
            (this.filter == "carry" && fund.id in FundInvestments.carryMap) ||
            (this.filter == "carryonly" && (fund.id in FundInvestments.carryMap &&
                !(fund.id in FundInvestments.commitMap))) ||
            (this.filter == "invest" && fund.id in FundInvestments.commitMap) ||
            this.filter == fund.status)
            return(true);
        else
            return(false);
    }

};  // FundTotalsConstructor




var angelReturnsByYear = {
    years: {},              // angel returns by year
    filteredYears: {},      // angel returns per year filtered
    newCosPerYear:  [],     // number of new companies invested in per year
    invsPerYear: [],        // total investments made per year
    ...
    add: function(input, isPayout) {
        if (isPayout)
            var d = new Date(input["payDate"]);
        else
            var d = new Date(input["inv_date"]);
        ...
        if (isPayout) {
            if (input["type"] == "cash")
                this.years[year].return_amount += Number(input["cash"]);
            else
                this.years[year].return_amount += Number(input["shares"])* Number(input["sharePrice"]);
        } else {
            /*if (!(inv.company_id in Companies.map))
                this.newCosPerYear[year]++;     this is wrong :( */
            if (year in this.invsPerYear)
                this.invsPerYear[year]++
            else
                this.invsPerYear[year] = 1;

            this.years[year].invest_amount += Number(input["invest_amount"]);
        }
    }
}
...
var fundReturnsByYear = {
    yearsIncCarry: {},      // fund returns by year including funds / carry
    yearsNoCarry: {},       // fund returns only for funds with investment (i.e which have an ROI)
    years: {},              //
    // funds
    funds: {},              // fund names mapped to returns by year (for the past fundPastYears years)
    fundYears: {},          // fund actual returns by year (no carry)
    fundPastYears: 10,       // we only go this many years in the past

    fundsInFuture: {},      // fund names mapped to predicted (new) returns by year from the current year until fundFutureYears
    fundYearsInFuture: {},  // predicted returns by year from (all) funds
    fundFutureYears: 10,
    ...
    clearFunds: function() {
        for (year in this.fundYears) {
            y = this.fundYears[year];
            y.invest_amount = 0;
            y.return_amount = 0;
        }
        for (fund in this.funds) {
            var years = this.funds[fund];
            for (year in years) {
                y = years[year];
                y.invest_amount = 0;
                y.return_amount = 0;
        ...
        for (fund in this.fundsInFuture) {
            var years = this.fundsInFuture[fund];
            for (year in years) {
                y = years[year];
                y.invest_amount = 0;
                y.return_amount = 0;
        ...
        for (year in this.fundYearsInFuture) {
            y = this.fundYearsInFuture[year];
            y.invest_amount = 0;
            y.return_amount = 0;
        }
    },
    ...
    calcFundReturns: function(fund) {

        var d, year, i, years;

        if (!checkFundsFilter(fund))        // only add the fund if it is not filtered
            return;

        if (!this.funds.hasOwnProperty(fund.id)) {
            years = this.funds[fund.id] = {};
        } else {
            years = this.funds[fund.id];
        }

        for (i=0; i<FundInvestments.list.length; i++) {
            var f = FundInvestments.list[i];
            if (f.fundID == fund.id) {
                d = new Date(f.invDate);
                year = d.getUTCFullYear();
                year = year.toString();
                if (f.type == "invest") {
                    if (!years.hasOwnProperty(year)) {
                        years[year] = Object.create(oneYearTotals);
                    }
                    if (!this.fundYears.hasOwnProperty(year)) {
                        this.fundYears[year] = Object.create(oneYearTotals);
                    }
                    this.fundYears[year].invest_amount += Number(f.investAmount);
                    years[year].invest_amount += Number(f.investAmount);
                }
            }
        ...
        for (i=0; i<FundPayouts.list.length; i++) {
            ...
            if (p.fundID == fund.id) {
                ...
                if (p.type == "cash") {
                    amount = Number(p.cash);
                    years[year].return_amount += amount;
                    this.fundYears[year].return_amount += amount;
                } else {
                    var amount =  Number(p.shares)* Number(p.sharePrice);
                    years[year].return_amount += amount;
                    this.fundYears[year].return_amount += amount;
                }
        ...
        //console.debug("returnsByYear.addFunds: (fund Name, id):" + angelParens(fund.name, fund.id))
        //console.debug("returnsByYear.addFunds: years dump: " + JSON.stringify(this.funds[fund.id]));
    },       // addFundActuals
    /*
     * this function goes through all of the actual dists and cap calls for a fund and add them.
     */
    addFundActuals: function(fund) {
    ...
    addFundFutures: function(fund) {
        ...
        if (!this.fundsInFuture.hasOwnProperty(fund.id)) {
            years = this.fundsInFuture[fund.id] = {};
        } else {
            years = this.fundsInFuture[fund.id];
        }
         ...
         * predicted returns
         */
        for (var y = CurrentYear; y < CurrentYear + this.fundFutureYears; y++) {

            if (!years.hasOwnProperty(y)) {
                years[y] = Object.create(oneYearTotals);
            }
            years[y].return_amount = fund.model.amountInYear(y);

            if (!this.fundYearsInFuture.hasOwnProperty(y)) {
                this.fundYearsInFuture[y] = Object.create(oneYearTotals);
            }
            this.fundYearsInFuture[y].return_amount += fund.model.amountInYear(y);      // add to total for year
        }
        /*if (!years.hasOwnProperty("left")) {
            years["left"] = Object.create(oneYearTotals);
        }
        if (!this.fundYearsInFuture.hasOwnProperty("left")) {
            this.fundYearsInFuture["left"] = Object.create(oneYearTotals);
        }*/
/*        years["left"].return_amount = fund.model.amountLeft();
        this.fundYearsInFuture["left"].return_amount += fund.model.amountLeft();   */
...
var PayoutsByYear = {
    years: [],
    addPayout: function(pay) {
        var d = new Date(pay["payDate"]);
        var year = d.getUTCFullYear();
        year = year.toString();
        console.debug("hi: " + year);
        if (!this.years.hasOwnProperty(year)) {
            this.years[year] = Object.create(oneYearTotals);
        }

        this.years[year].payout_amount += Number(pay["cash"]);
    }
};

var firstGraphicsCall = true;

var Precision = {
    decimals: -1,                // -1 means max precision, otherwise number of decimals to display
    digits: function() {         // default precision for display - if decim
        if (this.decimals == -1)
            return(4);
        else
    ...
    round: function (value) {
        if (this.decimals == -1)
            ...
            // console.debug("precision.round: " + angelParens(Math.round(value+'e'+this.decimals), 'e-'+this.decimals ) + Number(Math.round(value+'e'+this.decimals)+'e-'+this.decimals));
            return Number(Math.round(value+'e'+this.decimals)+'e-'+this.decimals);
        }
    }
};

var CompanyStatusFilter = "all";


/*
 * models an equity transaction
 */

function initializeModel(action, investorID, modelID, message) {

    // console.debug("initializeModel: action: "  + action);

    if (action == "loadSample")
        loadSampleModel('');
    else if (action == "converting") {
        document.getElementById('convertTable').style.display = 'block';
    } else if (action == "loadModel") {
        loadModelByID(modelID,'','');
        $('#saveButton').notify(message, {position:'right bottom', className:'success'});
    } else {
        // nothing to do, I think.
        console.debug("initialeModel: default");
    }
...
function modelEquity() {
    ...
    cleanUpOutput();        // start by clearing previous model output

    /*
     * Create the new convert objects and fill in Equity and those
     * objects with the latest input values
     */

    Equity.totalInvested         = Number($('#investAmount').val());
    Equity.yourInvestment        = Number($('#yourAmount').val());
    Equity.preMoneyVal           = Number($('#valuation').val());
    Equity.postOptionsPercent    = Number($('#opoolPost').val());

    console.debug("modelEquity equity: tot,your,val,shares,opts: " + Equity.totalInvested + "," + Equity.yourInvestment + "," + Equity.preMoneyVal + "," + Equity.preFDShares + "," + Equity.preUnallocOptions);

    if (!document.getElementById('valispre').checked)
        Equity.preMoneyVal -= Equity.totalInvested;
    ...
    if (Equity.preFDShares < (Equity.commonTotalEquity + Equity.founderTotalEquity))
        $.notify("More common shares than pre-money shares! Model will be incorrect.", "warn");



    /*****************************************************************/
    /*
     * Start by figuring out the number of fully diluted pre round shares and options
     */

    // totalFDpreShares = Equity.preIssuedShares/(1 - (Equity.preOptionsPercent/100));
    //var oSharesPre = totalFDpreShares -  Equity.preIssuedShares;
    console.debug("model equity totalFDpreShares =" + Equity.preFDShares);
    /*
     * and here is the guts. calculate the cap table using goal seeking since
     * there is lots of circular refs here and then update all the tables.
     */
    Equity.postOptions = calcCapTable(Equity.preFDShares);    // calc cap table

    updateFinancingSummary("post");                                    // update equity
    updateCapTable();
    updateEquationTable();
    updateForm();                                           // hidden fields for form submission (I think this is
                                                            // now unnecessary since the form isn't ever submitted



    $("#outputDiv").css("display","block");
    $("#financingSummary").css("display","block");
    $("#financingDetails").css("display","block");

}   // modelEquity

function modelConvertOnly() {
    // output
     ...
     * objects with the latest input values
     */
    Equity.totalInvested        = 0;    // VC investment
    Equity.yourInvestment       = 0;    // Angel investment
    Equity.postOptionsPercent   = 0;        // option percentage post financing
    // Equity.preUnallocOptions    =  Number($('#opoolPre').val());
    // Equity.preFDShares           = Number($('#sharesOut').val());


    console.debug("modelConvertOnly equity: tot,your,val,shares,opts: " + Equity.totalInvested + "," + Equity.yourInvestment + "," + Equity.preMoneyVal + "," + Equity.preFDShares + "," + Equity.preUnallocOptions);
    ...
    if (ConvertList.length == 0) {
        $.notify("No converts to model", "error");
        return;
    }


    if (Equity.preFDShares <= 0) {
        $("#dialogGetFDShares").dialog({
            resizable: false,
            height: 250,
            width: 250,
            modal: true,
            buttons: {
                Continue: function() {
                    Equity.preFDShares = Number($('#dialog-fullyDilutedShares').val());
                    $(this).dialog("close");
                    if (Equity.preFDShares <= 0) {
                        $.notify("Please set the number of pre-Money fully diluted shares in an Equity Round first.", "error");
                        return;
                    } else
                        proceed();
                }
    ...
    function proceed() {
        ...
        /*
         * see valuation to highest cap * 2 -- make sure discounts don't apply in almost every case
         */
        if (Equity.preFDShares < (Equity.commonTotalEquity + Equity.founderTotalEquity))
            $.notify("More common shares than pre-money shares! Model will be incorrect.", "warn");
        ...
        for (i=0; i < ConvertList.length; i++) {
            conv = ConvertList[i];
            if (Equity.preMoneyVal < conv.cap)
                Equity.preMoneyVal = conv.cap;
        }
        Equity.preMoneyVal = Equity.preMoneyVal * 2;
        // console.debug("modelconvertonly: premoneyVal is: " + Equity.preMoneyVal);
        ...
        // totalFDpreShares = Equity.preIssuedShares/(1 - (Equity.preOptionsPercent/100));
        //var oSharesPre = totalFDpreShares -  Equity.preIssuedShares;
        console.debug("modelConvertOnly totalFDpreShares =" + Equity.preFDShares);
        /*
         * and here is the guts. calculate the cap table using goal seeking since
         * there is lots of circular refs here and then update all the tables.
         */
        ...
        if (Equity.preUnallocOptions != 0) {
            Equity.postOptions = Equity.preUnallocOptions;
            Equity.postFDShares += Equity.postOptions;
        }

        console.debug("Equity.preUnallocOptions: " + Equity.preUnallocOptions);


        updateFinancingSummary("pre");                                    // update equity
        updateCapTable("pre");
        updateEquationTable();
        updateForm();                                           // hidden fields for form submission (I think this is
        // now unnecessary since the form isn't ever submitted
...
function modelInitialCap() {
    ...
    console.debug("modelInitialCap Equity: tot,your,val,shares,opts: " + Equity.totalInvested + "," + Equity.yourInvestment + "," + Equity.preMoneyVal + "," + Equity.preFDShares + "," + Equity.preUnallocOptions);
    ...
    function proceed() {
        ...
        console.debug("modelInitialCap totalFDpreShares =" + Equity.preFDShares);

        /*
         * Very simple, the pre shares and the post shares are the same!
         */

        Equity.postOptions = Equity.preUnallocOptions;
        Equity.postFDShares = Equity.preFDShares;


        console.debug("Equity.preUnallocOptions: " + Equity.preUnallocOptions);


        updateFinancingSummary("initial");                                    // update equity
        updateCapTable("initial");
        updateEquationTable();
        updateForm();                                           // hidden fields for form submission (I think this is
        // now unnecessary since the form isn't ever submitted
...
function countpreFDShares(modNum) {
    var founders = 0, otherCommon = 0, optionPool, optionsPromised, optionsUnissued, optionsGranted;

    optionPool = Number($('#optionPoolSizeForm'+modNum).val());
    optionsPromised = Number($('#promisedOptionsForm'+modNum).val());
    optionsGranted = Number($('#grantedOptionsForm'+modNum).val());
    optionsUnissued =Number($('#unissuedOptionsForm'+modNum).val());

    console.debug("countpreFDShares: optionPool,promised, unissued,granted: " +
                         optionPool+","+optionsPromised+","+optionsUnissued+","+optionsGranted);


    Equity.preUnallocOptions    = optionsUnissued;
    Equity.preOptionPoolSize    = optionPool;
    Equity.preGrantedOptions    = optionsGranted;
    Equity.prePromisedOptions    = optionsPromised;


    if (optionsUnissued == 0)
        optionPool = optionsPromised + optionsGranted;

    /*
     * deal with legacy case prior to optionpool. in this case only unissued was set.
     */
    if (optionPool == 0 && optionsUnissued != 0)
        optionPool = optionsUnissued;

    /*
     *  add founders and remaining common
     */
    for (i=0; i < Equity.numFounders; i++) {
        founders += Equity.founderEquity[i];
    }

    for (i=0; i < Equity.numOtherCommon; i++) {
        otherCommon += Equity.commonEquity[i];
    }

    Equity.preFDShares           = optionPool+founders+otherCommon;
    console.debug("countpreFDShares: optionPool,unissued,founders,otherCommon: " + optionPool+","+Equity.preUnallocOptions+","+founders+","+otherCommon);
    return;
}

/*
 * compare two models - model 1 and model 2
 */
function modelCompare() {
    ...
    var equity1 = calcModel("1");
    updateModelTable("1");

    Equity.reset();
    ConvertList = [];

    var equity2 = calcModel("2");
    updateModelTable("2");


    updateModelDifferenceTable(equity1, equity2);


    pieGraphEquity(equity1,'model1-chart');
    pieGraphEquity(equity2,'model2-chart');
...
function calcModel(modNum) {
    //equityModel = Object.create(Equity);

    createConvertList(modNum);
    createCommonLists(modNum);

    /* delete the appropriate lines above when confident that the new jquery formatting is working */

    Equity.totalInvested         = Number($('#investAmount'+modNum).val());
    Equity.yourInvestment        = Number($('#yourAmount'+modNum).val());
    Equity.preMoneyVal           = Number($('#valuation'+modNum).val());
    Equity.postOptionsPercent    = Number($('#opoolPost'+modNum).val());


    countpreFDShares(modNum);





    console.debug("calcModel equity: tot,your,val,shares,opts: " + Equity.totalInvested + "," + Equity.yourInvestment + "," + Equity.preMoneyVal + "," + Equity.preFDShares + "," + Equity.preUnallocOptions);


    if (!document.getElementById('valispre'+modNum).checked)
        Equity.preMoneyVal -= Equity.totalInvested;
     /*
     * and here is the guts. calculate the cap table using goal seeking since
     * there is lots of circular refs here and then update all the tables.
    ...
    /*
     * calculate total convert shares
     */
    Equity.convertShares = 0;
    Equity.totalYourShares = 0;
    for (i=0; i < ConvertList.length; i++) {
        conv = ConvertList[i];
        Equity.convertShares += conv.totalShares;
        Equity.totalYourShares += conv.yourShares;
    }
    Equity.totalYourShares += Equity.yourShares;        // include any shares angel bought in equity round

    var equityModel = Object.assign({}, Equity);
    console.debug("calcModel: options:" + equityModel.postOptions);

    console.debug("calcModel - totalFDpreShares =" + Equity.preFDShares);

    return(equityModel);

}

function calcModelFromID(modelID) {
    //equityModel = Object.create(Equity);

    var numConverts = 0;
    var url = "model?loadModel="+modelID;

    if (modelID == 0) {        // sample model

        Equity.totalInvested        = 2000000;
        Equity.yourInvestment       = 0;
        Equity.preMoneyVal          = 8000000;
        Equity.postOptionsPercent   = 10;
        Equity.preUnallocOptions    = 150000;
        Equity.preFDShares          = 10000000;

        conv1 = Object.create(Convert);
        conv1.type = "YCVC SAFE";
        conv1.cap = 10000000;
        conv1.totalInvested = 100000;
        ConvertList[0] = conv1;

        conv2 = Object.create(Convert);
        conv2.type = "SAFE";
        conv2.totalInvested = 1000000;
        conv2.yourInvestment = 50000;
        conv2.discount = 10;
        conv2.cap = 6000000;
        ConvertList[1] = conv2;
        ...
        $.get(url, function(data,status) {
            ...
            Equity.totalInvested        = Number(dict["vcInvestment"]);
            Equity.yourInvestment       = Number(dict["yourInvestment"]);
            Equity.preMoneyVal          = Number(dict["preValuation"]);
            Equity.postOptionsPercent   = Number(dict["postOptionsPercent"]);
            Equity.preUnallocOptions    = Number(dict["preMoneyUnallocOptions"]);
            Equity.preFDShares          = Number(dict["fdPreMoneyShares"]);
            ...
            for (i=0; i <numConverts; i++) {
                ...
                conv.yourInvestment  = Number(dict["CONV"+i+"convAmount"]);
                if (type == "YCVC SAFE")
                    conv.totalInvested   = 100000;
                else
                    conv.totalInvested   = Number(dict["CONV"+i+"convTotal"]);
                conv.discount        = Number(dict["CONV"+i+"discount"]);
                if (type == "YCVC SAFE")
                    conv.cap         = 10000000;
                else
                    conv.cap         = Number(dict["CONV"+i+"cap"]);
                conv.type         = type;

                if (conv.type == "Custom") {
                    conv.custom = dict["CONV"+i+"custom"];
                    conv.preexist = dict["CONV"+i+"preexist"];
                } else if (conv.type == "YCVC SAFE")
                    conv.preexist = dict["CONV"+i+"preexist"];
                else if (conv.type == "Manual") {
                    conv.totalShares = Number(dict["CONV"+i+"shares"]);
                    conv.price = Precision.round(conv.totalInvested / conv.totalShares);
                }
                ConvertList[i] = conv;
            }
            console.debug("calcModelfromID equity: tot,your,val,shares,opts: " + Equity.totalInvested + "," + Equity.yourInvestment + "," + Equity.preMoneyVal + "," + Equity.preFDShares + "," + Equity.preUnallocOptions);
            ...
            var equityModel = Object.assign({}, Equity);
            console.debug("calcModelbyid: options:" + equityModel.postOptions + " totalFDpreShares=" + Equity.preFDShares );
...
function initializeModelHomePage(investorID) {
     /*
      * place feedback on page
      */
    // console.debug("initmodelhomepage");
    initFeedback("right-bottom", investorID);
